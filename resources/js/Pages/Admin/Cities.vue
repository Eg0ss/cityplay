        ...
        <div class="space-y-8 lg:space-y-12">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                ...
                <div class="relative w-full lg:w-96 group">
                    ...
                    <input 
                        ...
                        class="w-full bg-white dark:bg-[#111113] border-2 dark:border-white/5 border-gray-200 rounded-2xl py-5 pl-16 pr-8 text-xs font-black uppercase tracking-widest focus:ring-0 focus:border-[#2fc276]/50 transition-all placeholder:text-gray-500/50 text-gray-900 dark:text-white"
                    />
                </div>

                <!-- Bouton d'action pour lancer ou annuler l'initialisation -->
                <button @click="showForm ? (showForm = false) : openCreateForm()" 
                    :class="showForm ? 'bg-red-500 shadow-lg text-white' : 'bg-[#2fc276] text-black shadow-lg'"
            ...
            <transition name="gaming-slide">
                <div v-if="showForm" class="dark:bg-[#1c183a] bg-white p-6 lg:p-12 rounded-[2.5rem] border dark:border-white/10 border-gray-200 shadow-2xl relative overflow-hidden">
                        ...
                        <transition name="step-fade" mode="out-in">
                            <div v-if="currentStep === 1" key="step1" class="space-y-8 py-4">
                                <div class="space-y-2">
                                    ...
                                    <h3 class="text-3xl font-black uppercase italic tracking-tighter dark:text-white">INITIALISATION DE LA <span class="text-[#2fc276]">CITE</span></h3>
                                </div>
                                <div class="space-y-4">
                                    <label class="block text-[10px] font-black uppercase tracking-[0.2em] dark:text-gray-400 text-gray-500 ms-2">Désignation de la Cité</label>
                                    <input 
                                        v-model="form.name" 
                                        type="text" 
                                        autofocus
                                        class="w-full text-2xl lg:text-4xl dark:bg-[#10101c]/60 bg-gray-50 border-none rounded-3xl py-8 px-8 focus:ring-4 focus:ring-[#2fc276]/20 dark:text-white text-gray-900 font-black italic tracking-tighter placeholder:opacity-20" 
                            ...
                            <div v-else-if="currentStep === 2" key="step2" class="space-y-8 py-4">
                                <div class="space-y-2">
                                    ...
                                    <h3 class="text-3xl font-black uppercase italic tracking-tighter dark:text-white">DEPARTEMENT <span class="text-[#2fc276]">RATACHE</span></h3>
                                </div>
                                <div class="space-y-4">
                                    <label class="block text-[10px] font-black uppercase tracking-[0.2em] dark:text-gray-400 text-gray-500 ms-2">Département (Optionnel)</label>
                                    <input 
                                ...
                                <div class="flex gap-4">
                                    <button @click="prevStep" class="px-8 py-6 rounded-2xl border-2 dark:border-white/10 border-gray-200 font-black uppercase tracking-widest text-xs dark:text-white hover:bg-white hover:text-black transition-all">RETOUR</button>
                            ...
                            <div v-else-if="currentStep === 3" key="step3" class="space-y-8 py-4">
                                <div class="space-y-2">
                                    ...
                                    <h3 class="text-3xl font-black uppercase italic tracking-tighter dark:text-white">DESCRIPTION DE LA <span class="text-[#2fc276]">CITÉ</span></h3>
                                </div>
                                <div class="space-y-4">
                                    <label class="block text-[10px] font-black uppercase tracking-[0.2em] dark:text-gray-400 text-gray-500 ms-2">Séquence d'introduction (Description - Optionnelle)</label>
                                    <textarea 
                                        v-model="form.description" 
                                        rows="4"
                                        autofocus
                                        class="w-full text-xl lg:text-2xl dark:bg-[#10101c]/60 bg-gray-50 border-none rounded-3xl py-8 px-8 focus:ring-4 focus:ring-[#2fc276]/20 dark:text-white text-gray-900 font-bold italic tracking-tight placeholder:opacity-20" 
                    ...
                    <div class="absolute bottom-4 left-8 text-[8px] font-black text-gray-500 uppercase tracking-[0.4em]">Step {{ currentStep }} / 03</div>
                </div>
            </transition>
            ...
            <div v-if="filteredCities.length > 0" class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                <div v-for="city in filteredCities" :key="city.id" 
                    class="dark:bg-[#1c183a]/40 bg-white border dark:border-white/10 border-gray-200 rounded-[2.5rem] overflow-hidden flex flex-col group hover:border-[#2fc276]/40 transition-all duration-500 shadow-xl hover:-translate-y-2">
                    <div class="p-8 space-y-6 flex-1">
                        <div class="flex justify-between items-start">
                            <!-- Remplacement de l'emoji par une icône SVG Pro -->
                            <div class="h-14 w-14 dark:bg-[#10101c]/60 bg-gray-50 rounded-2xl flex items-center justify-center border dark:border-white/10 border-gray-200 group-hover:border-[#2fc276]/30 group-hover:text-[#2fc276] transition-all duration-500">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 dark:text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 22V10a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v12"/><path d="M18 22H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h13a2 2 0 0 1 2 2v4.5"/><path d="M22 22V15a2 2 0 0 0-2-2h-2"/><rect width="4" height="4" x="6" y="12" rx="1"/><rect width="4" height="4" x="10" y="12" rx="1"/></svg>
                            </div>
                            <div class="flex flex-col items-end gap-2">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <button @click.stop="openEditForm(city)" class="p-2 rounded-lg bg-blue-50 border border-blue-100 dark:bg-blue-500/10 dark:border-blue-500/50 text-blue-600 dark:text-blue-400 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-500 dark:hover:text-white transition-all">
                                    ...
                                    <button @click.stop="confirmDelete(city)" :disabled="form.processing" class="p-2 rounded-lg bg-red-50 border border-red-100 dark:bg-red-500/10 dark:border-red-500/50 text-red-600 dark:text-red-400 hover:bg-red-600 hover:text-white dark:hover:bg-red-500 dark:hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] font-bold dark:text-gray-400 text-gray-500 uppercase tracking-widest mb-4 italic">{{ city.departement }}</p>
                            <p class="text-xs dark:text-gray-300 text-gray-600 line-clamp-3 leading-relaxed font-medium">
                                {{ city.description }}
                            </p>
                        </div>
                    </div>
                    <div class="p-2">
                        <Link :href="route('admin.cities.places', { city: city.id })" 
                            ...
                            class="w-full flex items-center justify-center gap-3 bg-gray-100 dark:bg-white/10 dark:text-white text-gray-700 py-6 rounded-[1.8rem] text-[10px] font-black uppercase tracking-widest hover:bg-[#2fc276] hover:text-black transition-all group-hover:shadow-[0_0_20px_rgba(47,194,118,0.2)]">
