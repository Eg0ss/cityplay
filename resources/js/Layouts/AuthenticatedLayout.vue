...
const props = defineProps({
    // Quand hideBottomNav=true (pendant ActiveRiddle), le bouton Jouer
    // de la bottom nav est désactivé pour éviter qu'un joueur ne quitte
    // sa session en cours par accident.
...
const confirmLogout = () => {
    confirm.require({
        ...
        accept: () => { router.post(route('logout')); }
    });
};
    ...
    <div class="min-h-screen font-sans bg-[#171235] text-white">
        ...
        <ConfirmDialog :autoFocus="false">
            <template #container="{ message, acceptCallback, rejectCallback }">
                <div class="bg-[#10101c] border-2 border-[#7751de] p-6 rounded-[1.5rem] shadow-[0_0_45px_rgba(119,81,222,0.45)] max-w-sm w-full mx-auto animate-pop-in text-center">
                    <AlertTriangle :size="40" class="text-[#ffc628] mx-auto mb-3" />
                    ...
                    <div class="flex gap-3 justify-center">
                        <button @click="rejectCallback"
                            class="px-5 py-3 rounded-xl font-black uppercase text-[9px] tracking-widest bg-[#1c183a] border border-[#2a245c] text-white hover:text-[#87d74e] transition-colors">
                            {{ message.rejectLabel || 'Annuler' }}
                        </button>
                        <button @click="acceptCallback"
                            class="btn-3d btn-3d-red px-5 py-3 text-[9px] tracking-widest shadow-[0_4px_0_#9e2318]">
        ...
        <nav class="fixed top-0 z-50 w-full border-b bg-[#10101c]/95 border-[#2a245c] backdrop-blur-md">
                ...
                <div class="flex h-14 sm:h-16 items-center justify-between">
                    ...
                    <div class="flex items-center gap-4">
                        <Link href="/dashboard" class="flex items-center gap-2 shrink-0">
                            ...
                            <span class="text-base font-black tracking-tighter uppercase italic hidden sm:block text-white">
                                City<span class="text-[#87d74e] text-glow-green">play</span>
                            </span>
                        </Link>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button @click="confirmLogout"
                            class="hidden sm:flex items-center gap-1.5 btn-3d btn-3d-red px-3 py-2 text-[9px] tracking-widest shadow-[0_3px_0_#9e2318]">
            ...
            <transition name="fade">
                <div v-if="isMobileMenuOpen"
                    class="sm:hidden border-t bg-[#10101c] border-[#2a245c] px-4 py-5 space-y-4">
                    ...
                    <nav class="flex flex-col gap-1">
                        <Link :href="route('dashboard')"
                            class="flex items-center gap-3 text-sm font-black uppercase tracking-widest py-3 px-3 rounded-xl hover:bg-[#1c183a] text-white transition-all">
                        ...
                        <button @click="confirmLogout"
                            class="flex items-center gap-3 text-sm font-black uppercase tracking-widest py-3 px-3 rounded-xl text-red-400 text-left w-full hover:bg-red-500/10 transition-all">
        ...
        <!-- CONTENU -->
        <div class="pt-14 sm:pt-16" :class="hideBottomNav ? 'pb-4' : 'pb-20 sm:pb-0'">
        ...
        <!--
            BOTTOM NAV MOBILE
            hideBottomNav=true → masquée pendant ActiveRiddle
            Pourquoi ? Le joueur ne doit pas pouvoir cliquer sur "Jouer"
            pour relancer une session pendant qu'une partie est en cours.
            Il doit passer par "Pause → Abandonner" pour quitter proprement.
        -->
        <nav v-if="!hideBottomNav"
            class="sm:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#10101c]/97 border-t border-[#2a245c] backdrop-blur-md"
